...
public class CarDao {
    ...
    public boolean updateTradeMark(TradeMark mark) {
        String query = "UPDATE [dbo].[trade_mark] SET [name] = ?, [logo_url] = ?, [privacy] = ?, [terms] = ? WHERE seller_id = ?";
    ...
    public boolean addNewTradeMark(TradeMark mark) {
        try {
            String query = "INSERT INTO trade_mark (name,logo_url,privacy,terms,seller_id) VALUES ( ?,?,?,?,?)";
            PreparedStatement ps = con.prepareStatement(query);
            ps.setString(1, mark.getName());
    ...
    public static void main(String[] args) {
        ...
        test.add("ccdsadasd");
        test.add("ccasdasdd");
        CarImage im = new CarImage(0, 6, test, 1012);
        carDAO.updateImages(im);
    ...
    public List<CarImage> getAllImgBySellerID(int user_id) {
        ...
        String query = "SELECT image_id,car_id, image_url,seller_id  FROM car_images WHERE seller_id = ?";
